...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SUPPORTED_PROVIDERS = [OFFICIAL_PROVIDER, ACCOUNT_PROVIDER]
    const SLOT_KEY = 'conversation.session.header.utilities'
    const BALANCE_ROUTE = '/plugins/dsh-deepseek-status/balance'
    const ACCOUNT_BALANCE_ROUTE = '/plugins/dsh-deepseek-status/account-balance'
    const STYLE_ID = 'dsh-deepseek-status-style'
    const DEFAULT_INTERVAL_MS = 300000
    const PRICING_REFRESH_MS = 1000
    ...
    // 2026 Chinese statutory holidays. DeepSeek charges the whole day as
    // off-peak (valley) on these dates, and also on adjusted working weekends.
    // Source: State Council holiday schedule / chinese-days@1.5.9.
    ...
    const CN_HOLIDAY_SET = new Set(CN_HOLIDAYS_2026)

    function dateKeyOf(year, monthIndex, dayOfMonth) {
    ...
    function beijingParts(ts) {
      ...
      return {
        ...
        mo,
        d,
        day: shifted.getUTCDay(), // 0 = Sunday, 6 = Saturday (Beijing time)
        dateKey: dateKeyOf(y, mo, d),
        h,
        m,
        s,
    ...
    // Weekends (including adjusted working weekends) and statutory holidays are
    // all-day valley.
    function isValleyDay(year, monthIndex, dayOfMonth, day) {
      return isWeekend(day) || CN_HOLIDAY_SET.has(dateKeyOf(year, monthIndex, dayOfMonth))
    }

    function isPeak(parts) {
      if (isValleyDay(parts.y, parts.mo, parts.d, parts.day)) return false
      const t = parts.h * 60 + parts.m
      return (t >= 9 * 60 && t < 12 * 60) || (t >= 14 * 60 && t < 18 * 60)
    }

    // Seconds from the start of the current Beijing day to the next workday
    // peak start (09:00). Scans forward past weekends and holidays, so a
    // Friday evening before a Monday holiday correctly counts through the
    // whole closed block.
    function secondsToNextWorkdayNine(parts) {
      const dayStart = Date.UTC(parts.y, parts.mo, parts.d)
      let candidate = dayStart + 24 * 60 * 60 * 1000
      for (;;) {
        ...
        if (!isValleyDay(y, mo, d, dow)) {
          return (candidate - dayStart) / 1000 + 9 * 60 * 60
        }
        candidate += 24 * 60 * 60 * 1000
      }
    }

    // Seconds until the current pricing period ends.
    function nextPeriodEndSeconds(parts) {
      const cur = parts.h * 3600 + parts.m * 60 + parts.s
      const valleyToday = isValleyDay(parts.y, parts.mo, parts.d, parts.day)

      if (valleyToday) return secondsToNextWorkdayNine(parts) - cur

      // Workday boundaries: 09:00-12:00 and 14:00-18:00 are peak.
      if (parts.h < 9) return 9 * 3600 - cur
      if (parts.h < 12) return 12 * 3600 - cur
      if (parts.h < 14) return 14 * 3600 - cur
      if (parts.h < 18) return 18 * 3600 - cur
      return secondsToNextWorkdayNine(parts) - cur
    }
    ...
    // Format balance for display. `compact` is used in the pill to keep it
    // short for very large balances; the full value is always kept in the
    // tooltip title.
    function formatBalance(value, compact = false) {
      const n = Number(value)
      if (!Number.isFinite(n)) return value
      const abs = Math.abs(n)
      if (compact && abs >= 1e8) {
        return (n / 1e8).toLocaleString('zh-CN', { maximumFractionDigits: 2 }) + '亿'
      }
      if (compact && abs >= 1e4) {
        return (n / 1e4).toLocaleString('zh-CN', { maximumFractionDigits: 2 }) + '万'
      }
      return n.toLocaleString('zh-CN', { maximumFractionDigits: 2 })
    }

    // Tracks the current session's model provider.
    // `deepseek-official` = API-key route, `deepseek-account` = signed-in account route.
    function useSessionProvider(ctx, sessionId) {
    ...
    // Account-platform wire locale, e.g. zh-CN -> zh_CN.
    ...
    // Currency symbol for a wire wallet currency.
    function currencySymbol(currency) {
      return String(currency) === 'USD' ? '$' : '¥'
    }

    // Render one wallet list as a tooltip fragment.
    function walletLine(label, wallets) {
      if (!Array.isArray(wallets) || wallets.length === 0) return ''
      const parts = wallets.map((w) => currencySymbol(w.currency) + formatBalance(String(w.amount)))
      return label + parts.join(' / ')
    }

    function BalancePill({ provider }) {
      const isAccount = provider === ACCOUNT_PROVIDER
      const eligible = SUPPORTED_PROVIDERS.indexOf(provider) !== -1
      ...
      const refresh = React.useCallback(() => {
        ...
        const url = isAccount
          ? ACCOUNT_BALANCE_ROUTE + '?locale=' + encodeURIComponent(accountLocale()) + '&tz=' + String(-new Date().getTimezoneOffset() * 60)
          : BALANCE_ROUTE
              ...
              if (d.ok === true) {
                return {
                  ...
                  last: {
                    ...
                    bonusWallets: Array.isArray(d.bonusWallets) ? d.bonusWallets : [],
                  },
                  error: '',
      ...
      let title
      if (last !== undefined) {
        const symbol = currencySymbol(last.currency)
        const amount = last.total ? symbol + formatBalance(last.total, true) : ''
        chip = React.createElement('span', { className: 'dsh-ds-text' },
          isAccount ? 'DeepSeek 账号余额 ' : 'DeepSeek 余额 ',
          React.createElement('span', { className: 'dsh-ds-value' }, amount),
        )
        if (refreshing) title = '正在刷新余额…'
        else if (error !== '') title = error + ' · 点击刷新按钮重试'
        else if (isAccount) {
          const parts = ['账号余额 ' + symbol + formatBalance(last.total)]
          const bonus = walletLine('赠送 ', last.bonusWallets)
          if (bonus !== '') parts.push(bonus)
          const wallets = walletLine('钱包 ', last.wallets)
          if (wallets !== '') parts.push(wallets)
          parts.push('上次刷新 ' + elapsedMs + 'ms')
          title = parts.join(' · ')
        } else {
          title = '总额 ' + symbol + formatBalance(last.total) + ' · 赠送 ' + last.granted + ' · 充值 ' + last.toppedUp + ' · 上次刷新 ' + elapsedMs + 'ms'
        }
      } else if (status === 'loading') {
        chip = React.createElement('span', { className: 'dsh-ds-text' }, isAccount ? '账号余额 …' : '余额 …')
        title = isAccount ? '正在获取 DeepSeek 账号余额…' : '正在获取 DeepSeek 余额…'
      } else {
        chip = React.createElement('span', { className: 'dsh-ds-text' }, isAccount ? '账号余额不可用' : '余额获取失败')
    ...
    function PeakValleyPill({ now }) {
      ...
      const peak = isPeak(b)
      const remainingSeconds = nextPeriodEndSeconds(b)
      const countdown = formatDuration(remainingSeconds)
      const title = peak
        ? `DeepSeek 高峰时段 · 当前时段剩余 ${countdown} · 北京时间 ${b.text}`
        : `DeepSeek 空闲时段（半价）· 当前时段剩余 ${countdown} · 北京时间 ${b.text}`
      ...
      return React.createElement(
        ...
        React.createElement('span', { className: 'dsh-ds-text dsh-pv-text' }, peak ? '时段：峰' : '时段：谷'),
        React.createElement('span', { className: 'dsh-pv-time' }, countdown),
      )
    }
      ...
      apply(ctx) {
        ...
        slots.inject(SLOT_KEY, () => slots.register(
          ...
          function DeepSeekStatusChip(props) {
            ...
            const provider = useSessionProvider(ctx, sessionId)
            ...
            if (SUPPORTED_PROVIDERS.indexOf(provider) === -1) return null

            return React.createElement(
              'span',
              { className: 'dsh-ds-status' },
              React.createElement(BalancePill, { provider }),
