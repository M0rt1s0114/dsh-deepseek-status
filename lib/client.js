...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Weekends (Saturday and Sunday) are always valley after the new rule.
    function isWeekend(day) {
      return day === 0 || day === 6
    }

    function isPeak(day, h, m) {
      if (isWeekend(day)) return false
      const t = h * 60 + m
      return (t >= 9 * 60 && t < 12 * 60) || (t >= 14 * 60 && t < 18 * 60)
    }

    // Minutes until the current pricing period ends.
    // Weekends are a continuous valley block from Friday 18:00 to Monday 09:00.
    function nextPeriodEndMinute(day, h, m) {
      const cur = h * 60 + m

      if (isWeekend(day)) {
        const daysUntilMonday = (1 - day + 7) % 7 // Saturday -> 2, Sunday -> 1
        return daysUntilMonday * 24 * 60 + 9 * 60 - cur
      }

      // Friday evening after the last peak end: valley runs to Monday 09:00.
      if (day === 5 && cur >= 18 * 60) {
        return 3 * 24 * 60 + 9 * 60 - cur
      }

      if (isPeak(day, h, m)) {
        return (cur < 12 * 60 ? 12 * 60 : 18 * 60) - cur
      }

      // Normal weekday valley.
      if (cur < 9 * 60) return 9 * 60 - cur
      if (cur < 14 * 60) return 14 * 60 - cur
      return 24 * 60 + 9 * 60 - cur
    }
    ...
    function PeakValleyPill({ now }) {
      ...
      const peak = isPeak(b.day, b.h, b.m)
      const remainingMinutes = nextPeriodEndMinute(b.day, b.h, b.m)
      const remainingSeconds = remainingMinutes * 60 - b.s
