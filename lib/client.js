...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PeakValleyPill({ now }) {
      ...
      const remaining = nextMinute - currentMinute
      const countdown = formatCountdown(remaining)
      const label = peak ? '距谷' : '距峰'
      const title = peak
        ? `DeepSeek 高峰时段 · 北京时间 ${b.text} · ${countdown}后进入空闲半价`
        : `DeepSeek 空闲时段（半价）· 北京时间 ${b.text} · ${countdown}后进入高峰`
      ...
      return React.createElement(
        ...
        React.createElement('span', { className: 'dsh-pv-time' }, label + ' ' + countdown),
      )
    }
