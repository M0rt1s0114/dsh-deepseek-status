// dsh-peak-valley client half (no-build bundle).
//
// Renders two pills in the DSH session header for official DeepSeek sessions:
//   1. DeepSeek API balance (fetched from our own local route)
//   2. DeepSeek peak/valley pricing status (computed locally, Beijing time)
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const BALANCE_ROUTE = '/plugins/dsh-peak-valley/balance'
    ...
    return {
      ...
      apply(ctx) {
        const ensureStyle = () => {
          ...
          el.id = STYLE_ID
          el.setAttribute('data-plugin', 'dsh-peak-valley')
          el.textContent = css
          document.head.appendChild(el)
          return () => { el.remove() }
        }
        ctx.effect(ensureStyle, 'dsh-peak-valley: styles')
